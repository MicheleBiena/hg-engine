        ...
        for line in f:
            if len(line.split()) > 1:
                ...
                if 'SPECIES' in test and not '_START' in test and not '_SPECIES_H' in test and not '_NUM (' in line and not 'MAX_' in test:
                    ...
                    canonicalSpeciesMaxIndex = index-1
                elif 'SPECIES_MAX_MON_NUM' in test:
                    speciesMaxIndex = index-1
    return species_dict, speciesMaxIndex or canonicalSpeciesMaxIndex
...
def write_species_constants_inc(max_num_species, output_path):
    os.makedirs(os.path.dirname(output_path), exist_ok=True)

    with open(output_path, "w") as f:
        f.write("// DO NOT MODIFY THIS FILE! Autogenerated by build_learnsets.py\n\n")
        f.write(f"NUM_OF_MONS equ {max_num_species}")
...
if __name__ == "__main__":
    ...
    [species_dict, base_species_max_index] = load_species_header("include/constants/species.h")
    ...
    if any([args.machineout, args.levelupout, args.eggout, args.tutorout, args.constsout]):
        ...
        if args.constsout:
            ...
            print(f"{base_species_max_index} is the max species index")
            write_species_constants_inc(base_species_max_index, "armips/include/generated/species.s")
