...
NO_OVERWORLD_FORMS_PATH = "data/graphics/no_overworld_forms.txt"
NO_OVERWORLD_PLACEHOLDER = "data/graphics/sprites/none/overworld.png"
...
def GrabNoOverworldForms(speciesDict: dict) -> set:
    noOverworldForms = set()
    if not os.path.exists(NO_OVERWORLD_FORMS_PATH):
        return noOverworldForms

    with open(NO_OVERWORLD_FORMS_PATH) as f:
        for line in f:
            species = line.split("#", 1)[0].strip()
            if not species:
                continue
            if species not in speciesDict:
                raise ValueError("Unknown species in {0}: {1}".format(NO_OVERWORLD_FORMS_PATH, species))
            noOverworldForms.add(species)
...
def GenMakefile(outputPath: str, speciesDict: dict):
    ...
    noOverworldForms = GrabNoOverworldForms(speciesDict)
# header
...
# entries
    for entry in speciesDict:
        ...
        btxSrcs = []
        btxDeps = []
        overworldPath = "data/graphics/sprites/" + speciesName + "/overworld.png"
        btxPath = overworldPath
        if entry in noOverworldForms:
            btxPath = NO_OVERWORLD_PLACEHOLDER
        if os.path.exists(btxPath):
            output.write(convertedBtx + ".btx0: " + btxPath + "\n" + btx_gen_format)
            btxSrcs.append(btxPath)
            btxDeps.append(convertedBtx + ".btx0")
        ...
        for i in range(0, len(filterList)):
            ...
            overworldFormPath = "data/graphics/sprites/" + speciesName + "/overworld_" + entrySuffix.lower() + ".png"
            if not os.path.exists(overworldFormPath):
                continue
            output.write(outputNameStr + ": " + overworldFormPath + "\n" + btx_gen_format)
            btxSrcs.append(overworldFormPath)
            btxDeps.append(outputNameStr)
        if btxSrcs:
            output.write("ALL_OVERWORLDS_SRCS += " + " ".join(btxSrcs) + "\n")
            output.write("ALL_OVERWORLDS_OBJS += " + " ".join(btxDeps) + "\n")
