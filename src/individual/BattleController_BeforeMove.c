...
void __attribute__((section(".init"))) BattleController_BeforeMove(struct BattleSystem *bsys, struct BattleStruct *ctx)
{
    ...
    case BEFORE_MOVE_END: {
        ...
        if (IsValidParentalBondMove(bsys, ctx, FALSE) && ctx->loop_hit_check != 0xFD) {
            ctx->multiHitCount = 2;
...
static void BattleController_StartFormChange(struct BattleSystem *bsys, struct BattleStruct *ctx, int battlerId, int formNo)
{
...
void BattleController_CheckStanceChange(struct BattleSystem *bsys, struct BattleStruct *ctx)
{
    ...
    if (ctx->battlemon[attacker].ability != ABILITY_STANCE_CHANGE || ctx->battlemon[attacker].species != SPECIES_AEGISLASH) {
        return;
    }

    if (ctx->current_move_index == MOVE_KINGS_SHIELD && ctx->battlemon[attacker].form_no == 1) {
        BattleController_StartFormChange(bsys, ctx, attacker, 0);
    } else if (ctx->moveTbl[ctx->current_move_index].power != 0 && ctx->battlemon[attacker].form_no == 0) {
        BattleController_StartFormChange(bsys, ctx, attacker, 1);
    }
}
...
void BattleController_CheckSinfaeChange(struct BattleSystem *bsys, struct BattleStruct *ctx)
{
    ...
    if (ctx->battlemon[attacker].species != SPECIES_SINFAE || ctx->current_move_index != MOVE_TAIL_WHIP) {
        return;
    }
...
BOOL CanHitThroughProtect(struct BattleStruct *ctx, int attacker, int defender)
{
    ...
    if (moveEffect == MOVE_EFFECT_REMOVE_PROTECT
        ...
        || moveEffect == MOVE_EFFECT_IGNORE_PROTECT
...
BOOL BattleController_CheckMagicBounceMagicCoat(struct BattleSystem *bw, struct BattleStruct *sp, BOOL isMagicBounce)
{
    ...
    if (!sp->magicBounceContext.isActive && (sp->moveTbl[sp->current_move_index].flag & FLAG_MAGIC_COAT)) {
        ...
        for (int i = 0; i < client_set_max; i++) {
            ...
            if (target == RANGE_SINGLE_TARGET) { // Glare, etc in Doubles
